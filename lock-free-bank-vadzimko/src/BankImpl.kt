...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        var fromRef = AtomicReference<AcquiredAccount>()
        var toRef = AtomicReference<AcquiredAccount>()
        ...
        override fun invokeOperation() {
            ...
            fromRef.compareAndSet(null, from)
            toRef.compareAndSet(null, to)
            ...
            when {
                ...
                to.amount + amount > MAX_AMOUNT -> errorMessage = "Overflow"
                    ...
                    val updatedFrom = AcquiredAccount(from.amount - amount, this)
                    val updatedTo = AcquiredAccount(to.amount + amount, this)
                    accounts[fromIndex].compareAndSet(from, updatedFrom)
                    accounts[toIndex].compareAndSet(to, updatedTo)
                }
            }
